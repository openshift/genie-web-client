...
const PLUGIN_TEMPLATE_NAME = 'genie-web-client';
...
const deleteHelmChart = (path: string) => {
  cy.exec(
    `cd ../../genie-web-client && ${path} uninstall ${PLUGIN_TEMPLATE_NAME} -n ${PLUGIN_TEMPLATE_NAME} && oc delete namespaces ${PLUGIN_TEMPLATE_NAME}`,
    {
      failOnNonZeroExit: false,
    },
  ).then((result) => {
...
describe('Console plugin template test', () => {
  before(() => {
    ...
    if (!isLocalDevEnvironment) {
      ...
      cy.exec('cd ../../genie-web-client && ./install_helm.sh', {
        failOnNonZeroExit: false,
      }).then((result) => {
