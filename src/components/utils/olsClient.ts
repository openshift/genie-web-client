...
class DefaultStreamingHandler implements ISimpleStreamingHandler<string | StreamingEvent> {
  ...
  private processJsonEvent(
    ...
    if (isTokenEvent(event)) {
      ...
      if ((event.data as any).role === 'tool_execution') {
        hasUpdate = true;
  ...
  private async handleErrorResponse(response: Response): Promise<never> {
    ...
    try {
      ...
      if (typeof errorBody.detail === 'string') {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      } else if ((errorBody.detail as any)?.response) {
        // ErrorResponse format
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        message = (errorBody.detail as any).response;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      } else if ((errorBody.detail as any)?.cause) {
        // ErrorResponse format
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        message = (errorBody.detail as any).cause;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
