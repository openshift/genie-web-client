...
- [Mocking and hooks](#mocking-and-hooks)
- [Timing and async operations](#timing-and-async-operations)
...
```javascript
import { render, screen } from 'src/unitTestUtils';
```
...
For every file that exports at least one React component, there is a corresponding test file in the same directory as the component file. The file should have the same name, but ending with `.test.jsx`, `.test.tsx`, `.test.js`, or `.test.ts`. Do not use `.spec.` for file names.
...
Create meaningful test fixtures in separate fixture files (e.g., `*.mock.ts`) when you have complex mock data or setup that is reused across multiple tests.
...
Jest allows the use of both the `test` and `it` methods to define a test. All tests should use the `it` method. When skipping tests, use `it.skip` instead of `xit`.
...
it.skip('is accessible when external library bug is fixed', async () => {
  /* ... */
});
```
...
Focus on testing what the component does (behavior) rather than how it does it (implementation). This makes tests more resilient to refactoring and ensures you're testing the right things.

### Test error states, loading states, and edge cases

Make sure to test:
- Error states (when API calls fail, validation errors, etc.)
...
```javascript
import { render, screen, checkAccessibility, user } from 'src/unitTestUtils';
...
it('displays welcome banner when "welcome" button is pressed AND is accessible', async () => {
  // Arrange
  const alertMessage = 'Welcome to our site Kim!';
  const { container, user } = render(<MySampleComponent name="Kim" />);
  expect(screen.queryByRole('alert', { name: alertMessage })).not.toBeInTheDocument();
...
Use `beforeEach`/`afterEach` only for common setup/cleanup tasks that don't involve rendering (e.g., resetting mocks, clearing timers).
...
In addition, jest will automatically clear mocks after each test to ensure that mocks don't bleed between tests. See [jest documentation](https://jestjs.io/docs/configuration#clearmocks-boolean) for more information. While manually calling `jest.clearAllMocks()` in a beforeEach or afterEach block is not necessary, it causes no harm and can be added to remind future coders of a given unit test file that clearing mocks is important between tests.

## Use `user` helper object

When testing user interactions like typing or clicking, use the `user` helper that is exported from `src/unitTestUtils`. This object has a configuration that works best with the code base. Do not directly use `userEvent` or `fireEvent`.

```javascript
import { render, screen, user } from 'src/unitTestUtils';
...
When RTL simulates a user typing into a textbox, it does not empty the textbox first. Always clear the textbox before simulating typing using `await user.clear()`.
...
If there is a state change in a component (either on mount or due to a user action) there may be a short time delay before the component is rerendered. This can cause tests to fail. RTL has two ways to wait for an expect statement to pass - `waitFor` and `findBy` (or `findAllBy`). The `findBy` or `findAllBy` selectors are preferred to ensure consistency across tests.

Use `findBy*` for async elements; use `waitFor` only when `findBy` is not appropriate (e.g., when waiting for a function to be called or a condition that doesn't involve DOM elements).
...
NOTE: There is an existing [RTL bug](https://github.com/testing-library/react-testing-library/issues/865) where if you save an element to a variable when using `findBy` in a later expect, this may cause intermittent unit test failures on CI.
...
Use descriptive assertion messages when failures might be unclear, especially when testing multiple similar elements.
...
The React Testing Library has added [custom matchers](https://github.com/testing-library/jest-dom#custom-matchers) that you can use with your expect statements. For example, you can verify that a checkbox has been checked using `toBeChecked()`.
...
Keep mocks simple and focused. **Do not mock child React components unless it is absolutely necessary.** Since RTL's philosophy is to test components as users would interact with them, mocking child components breaks this approach and reduces test confidence. Only mock child components when:

- The child component has external dependencies that cannot be easily set up in tests (e.g., complex third-party libraries)
- The child component causes side effects that would interfere with testing (e.g., network requests, timers)
- The child component is extremely slow to render and would make tests impractical

When mocking is necessary, use `jest.spyOn` for specific methods; use `jest.mock` at module level for consistency.

Avoid try/catch blocks in tests - let errors propagate naturally so they can be properly caught by the test framework.
...
Use `jest.useFakeTimers()`/`jest.useRealTimers()` for time-dependent tests. Always restore timers in `afterEach`/`afterAll`:
